...
public class CatAndMouseGame extends Thread {
	...
	public void runGame() {
		...
		/* File file = new File('/Users/tanshi/SIP2019/catPositions.csv'); 
    	try { 
			...
			for (int i=0; i<len(cLats); i++) {
				int[] data1 = {cLats[i], cLongs[i]};
		...
		File file = new File('/Users/tanshi/SIP2019/mousePositions.csv'); 
    	try { 
			// create FileWriter object with file as parameter 
			FileWriter outputfile = new FileWriter(file); 
			...
			for (int i=0; i<len(mLats); i++) {
				int[] data1 = {mLats[i], mLongs[i]};
				writer.writeNext(data1)
			}
